...
class FooterSettingsViewModel @Inject constructor(
    ...
    private val _footerData = MutableLiveData<TenantFooter?>()
    ...
    fun loadFooterSettings(sessionId: String) {
        viewModelScope.launch {
            ...
            try {
                ...
                result.onSuccess { footer ->
                    _footerData.value = footer
                }.onFailure { throwable ->
                    // If no footer exists yet, create an empty one
                    if (throwable.message?.contains("No footer found") == true) {
                        ...
                        _error.value = "Failed to load footer: ${throwable.message}"
                    }
                }
            } catch (e: Exception) {
    ...
    fun saveFooterSettings(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (tenantId <= 0) {
                    _saveResult.value = Result.failure(Exception("No tenant configured"))
                    return@launch
                }
                ...
                result.onSuccess {
                    ...
                    _saveResult.value = Result.failure(throwable)
                }
            } catch (e: Exception) {
